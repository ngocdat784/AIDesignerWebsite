...
async function main() {
    ...
    try {
        ...
        const createdUser = getData(createResponse);
        if (!createdUser ||
        ...
        // 2. GET BY ID
        // ==========================================
        const getByIdResponse = await request("GET", `/users/${testUserId}`);
        verifyApiResponse(getByIdResponse);
        const userById = getData(getByIdResponse);
        if (!userById ||
        ...
        // 3. GET BY EMAIL
        // ==========================================
        const getByEmailResponse = await request("GET", `/users/email/${encodeURIComponent(testEmail)}`);
        verifyApiResponse(getByEmailResponse);
        const userByEmail = getData(getByEmailResponse);
        if (!userByEmail ||
        ...
        // 4. GET ALL
        // ==========================================
        const getAllResponse = await request("GET", "/users");
        verifyApiResponse(getAllResponse);
        const allUsers = getData(getAllResponse);
        if (!Array.isArray(allUsers)) {
            throw new Error("GET ALL did not return an array.");
        }
        const foundUser = allUsers.find((user) => user.id === testUserId);
        if (!foundUser) {
            throw new Error("GET ALL did not contain the test user.");
        }
        console.log("GET ALL verified.");
        // ==========================================
        // 5. UPDATE
        // ==========================================
        const updateResponse = await request("PATCH", `/users/${testUserId}`, {
            name: "Updated API Test User",
            role: "CREATOR",
        });
        verifyApiResponse(updateResponse);
        const updatedUser = getData(updateResponse);
        if (!updatedUser ||
            updatedUser.name !==
        ...
        // 6. VERIFY UPDATE
        // ==========================================
        const verifyUpdateResponse = await request("GET", `/users/${testUserId}`);
        verifyApiResponse(verifyUpdateResponse);
        const verifiedUser = getData(verifyUpdateResponse);
        if (!verifiedUser ||
            verifiedUser.name !==
                "Updated API Test User" ||
        ...
        const validationResponse = await fetch(`${BASE_URL}/users`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
            },
        ...
        if (validationResponse.status !== 400) {
            throw new Error(`Expected validation status 400, got ${validationResponse.status}`);
        }
        if (!validationData ||
            typeof validationData !== "object") {
            throw new Error("Validation error response is invalid.");
        }
        const validationError = validationData;
        if (validationError.success !== false) {
            throw new Error("Validation error must have success=false.");
        }
        if (validationError.statusCode !== 400) {
            throw new Error("Validation error must have statusCode=400.");
        }
        if (!validationError.message) {
            throw new Error("Validation error is missing message.");
        }
        console.log("VALIDATION ERROR verified.");
        // ==========================================
        // 8. DELETE
        // ==========================================
        const deleteResponse = await request("DELETE", `/users/${testUserId}`);
        verifyApiResponse(deleteResponse);
        const deletedUser = getData(deleteResponse);
        if (!deletedUser ||
            deletedUser.id !== testUserId) {
            throw new Error("DELETE returned invalid user.");
        }
        console.log("DELETE verified.");
        // ==========================================
        // 9. VERIFY DELETE
        // ==========================================
        const verifyDeleteResponse = await fetch(`${BASE_URL}/users/${testUserId}`);
        console.log(`\nGET /users/${testUserId}`);
        console.log(`Status: ${verifyDeleteResponse.status}`);
        const deletedText = await verifyDeleteResponse.text();
        let deletedData;
        try {
        ...
        if (verifyDeleteResponse.status !== 404) {
            throw new Error("VERIFY DELETE expected HTTP 404.");
        }
        if (!deletedData ||
            typeof deletedData !== "object") {
            throw new Error("VERIFY DELETE returned invalid error response.");
        }
        const deleteError = deletedData;
        if (deleteError.success !== false) {
            throw new Error("VERIFY DELETE error must have success=false.");
        }
        if (deleteError.statusCode !== 404) {
            throw new Error("VERIFY DELETE error must have statusCode=404.");
        }
        if (!deleteError.message) {
            throw new Error("VERIFY DELETE error is missing message.");
        }
