...
async function main() {
    ...
    try {
        ...
        await request("DELETE", `/templates/${testTemplateId}`);
        ...
        // 1. CREATE TEST USER
        // ==========================================
        const createUserResult = await request("POST", "/users", {
            id: testUserId,
        ...
        // 2. CREATE TEMPLATE
        // ==========================================
        const createTemplateResult = await request("POST", "/templates", {
            id: testTemplateId,
            slug: testSlug,
            title: "API Test Template",
            description: "Template created for API testing.",
            thumbnail: "/images/api-test-template.png",
            images: [
        ...
        // 3. GET BY ID
        // ==========================================
        const getByIdResult = await request("GET", `/templates/${testTemplateId}`);
        ...
        // 4. GET BY SLUG
        // ==========================================
        const getBySlugResult = await request("GET", `/templates/slug/${encodeURIComponent(testSlug)}`);
        if (getBySlugResult.status !== 200 ||
        ...
        // 5. GET BY AUTHOR
        // ==========================================
        const getByAuthorResult = await request("GET", `/templates/author/${encodeURIComponent(testUserId)}`);
        if (getByAuthorResult.status !== 200 ||
            !Array.isArray(getByAuthorResult.data)) {
            throw new Error("GET BY AUTHOR failed.");
        }
        const templateByAuthor = getByAuthorResult.data.find((template) => template.id === testTemplateId);
        if (!templateByAuthor) {
            throw new Error("GET BY AUTHOR did not return the test template.");
        }
        console.log("GET BY AUTHOR verified.\n");
        // ==========================================
        // 6. GET BY CATEGORY
        // ==========================================
        const getByCategoryResult = await request("GET", "/templates/category/Dashboard");
        if (getByCategoryResult.status !== 200 ||
            !Array.isArray(getByCategoryResult.data)) {
            throw new Error("GET BY CATEGORY failed.");
        }
        const templateByCategory = getByCategoryResult.data.find((template) => template.id === testTemplateId);
        if (!templateByCategory) {
            throw new Error("GET BY CATEGORY did not return the test template.");
        }
        console.log("GET BY CATEGORY verified.\n");
        // ==========================================
        // 7. GET ALL
        // ==========================================
        const getAllResult = await request("GET", "/templates");
        if (getAllResult.status !== 200 ||
            !Array.isArray(getAllResult.data)) {
            throw new Error("GET ALL failed.");
        }
        const templateInAll = getAllResult.data.find((template) => template.id === testTemplateId);
        if (!templateInAll) {
            throw new Error("GET ALL did not return the test template.");
        }
        console.log("GET ALL verified.\n");
        // ==========================================
        // 8. UPDATE
        // ==========================================
        const updateResult = await request("PATCH", `/templates/${testTemplateId}`, {
        ...
        // 9. VERIFY UPDATE
        // ==========================================
        const verifyUpdateResult = await request("GET", `/templates/${testTemplateId}`);
        if (verifyUpdateResult.status !== 200 ||
        ...
        // 10. DELETE TEMPLATE
        // ==========================================
        const deleteTemplateResult = await request("DELETE", `/templates/${testTemplateId}`);
        if (deleteTemplateResult.status !== 200 ||
        ...
        // ==========================================
        const verifyTemplateDeleteResult = await request("GET", `/templates/${testTemplateId}`);
        if (verifyTemplateDeleteResult.status !== 200 ||
            verifyTemplateDeleteResult.data !== null) {
            throw new Error("VERIFY TEMPLATE DELETE failed.");
        }
        console.log("VERIFY TEMPLATE DELETE verified.\n");
        // ==========================================
        // 12. DELETE TEST USER
        // ==========================================
        const deleteUserResult = await request("DELETE", `/users/${testUserId}`);
        ...
        // ==========================================
        const verifyUserDeleteResult = await request("GET", `/users/${testUserId}`);
        if (!((verifyUserDeleteResult.status === 200 && verifyUserDeleteResult.data === null) ||
            verifyUserDeleteResult.status === 404)) {
            throw new Error("VERIFY TEST USER DELETE failed.");
        }
        console.log("VERIFY TEST USER DELETE verified.\n");
        // ==========================================
        // FINAL RESULT
        // ==========================================
        console.log("TEMPLATE API TEST PASSED.");
    }
    catch (error) {
