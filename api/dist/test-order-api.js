...
async function main() {
    ...
    try {
        ...
        await request("DELETE", `/orders/${testOrderId}`);
        ...
        if (!Array.isArray(createOrder.data?.items) ||
            createOrder.data.items.length !== 2) {
            throw new Error("CREATE ORDER did not return expected order items.");
        }
        ...
        // GET BY ID
        // =========================================================
        const getById = await request("GET", `/orders/${testOrderId}`);
        ...
        if (getById.data?.user?.id !== testUserId) {
            throw new Error("GET BY ID did not return correct user relation.");
        }
        if (!getById.data?.billing) {
            throw new Error("GET BY ID did not return billing relation.");
        }
        if (!Array.isArray(getById.data?.items) ||
            getById.data.items.length !== 2) {
            throw new Error("GET BY ID did not return order items.");
        }
        console.log("GET BY ID verified.\n");
        // =========================================================
        // GET BY USER ID
        // =========================================================
        const getByUserId = await request("GET", `/orders/user/${testUserId}`);
        ...
        const foundByUser = getByUserId.data.some((order) => order.id === testOrderId);
        if (!foundByUser) {
            throw new Error("GET BY USER ID did not return test order.");
        }
        console.log("GET BY USER ID verified.\n");
        // =========================================================
        // GET BY STATUS
        // =========================================================
        const getByStatus = await request("GET", "/orders/status/PENDING");
        ...
        const foundByStatus = getByStatus.data.some((order) => order.id === testOrderId &&
            order.status === "PENDING");
        if (!foundByStatus) {
            throw new Error("GET BY STATUS did not return test order.");
        }
        console.log("GET BY STATUS verified.\n");
        // =========================================================
        // GET ALL
        // =========================================================
        const getAll = await request("GET", "/orders");
        ...
        const foundInAll = getAll.data.some((order) => order.id === testOrderId);
        if (!foundInAll) {
            throw new Error("GET ALL did not return test order.");
        }
        console.log("GET ALL verified.\n");
        // =========================================================
        // PATCH
        // =========================================================
        const updateOrder = await request("PATCH", `/orders/${testOrderId}`, {
            status: "PAID",
        ...
        if (updateOrder.data?.status !== "PAID") {
            throw new Error("UPDATE ORDER did not update status.");
        }
        if (updateOrder.data?.paymentMethod !== "paypal") {
            throw new Error("UPDATE ORDER did not update paymentMethod.");
        }
        if (updateOrder.data?.discount !== 15) {
            throw new Error("UPDATE ORDER did not update discount.");
        }
        if (updateOrder.data?.total !== 63) {
            throw new Error("UPDATE ORDER did not update total.");
        }
        console.log("UPDATE verified.\n");
        // =========================================================
        // VERIFY PATCH
        // =========================================================
        const verifyUpdate = await request("GET", `/orders/${testOrderId}`);
        if (verifyUpdate.status !== 200) {
            throw new Error("VERIFY UPDATE GET failed.");
        }
        if (verifyUpdate.data?.status !== "PAID") {
            throw new Error("VERIFY UPDATE: status was not persisted.");
        }
        if (verifyUpdate.data?.paymentMethod !== "paypal") {
            throw new Error("VERIFY UPDATE: paymentMethod was not persisted.");
        }
        if (verifyUpdate.data?.discount !== 15) {
            throw new Error("VERIFY UPDATE: discount was not persisted.");
        }
        if (verifyUpdate.data?.total !== 63) {
            throw new Error("VERIFY UPDATE: total was not persisted.");
        }
        // Relations must still exist after update.
        if (!verifyUpdate.data?.billing) {
            throw new Error("VERIFY UPDATE: billing relation missing.");
        }
        if (!Array.isArray(verifyUpdate.data?.items) ||
            verifyUpdate.data.items.length !== 2) {
            throw new Error("VERIFY UPDATE: order items missing.");
        }
        if (verifyUpdate.data?.user?.id !== testUserId) {
            throw new Error("VERIFY UPDATE: user relation incorrect.");
        }
        console.log("VERIFY UPDATE verified.\n");
        // =========================================================
        // DELETE ORDER
        // =========================================================
        const deleteOrder = await request("DELETE", `/orders/${testOrderId}`);
        if (deleteOrder.status !== 200) {
            throw new Error("DELETE ORDER failed.");
        }
        if (deleteOrder.data?.id !== testOrderId) {
            throw new Error("DELETE ORDER returned incorrect order.");
        }
        console.log("DELETE ORDER verified.\n");
        // =========================================================
        // VERIFY ORDER DELETE
        // =========================================================
        const verifyOrderDelete = await request("GET", `/orders/${testOrderId}`);
        if (verifyOrderDelete.status !== 200) {
            throw new Error("VERIFY ORDER DELETE GET failed.");
        }
        ...
        // DELETE TEST USER
        // =========================================================
        const deleteUser = await request("DELETE", `/users/${testUserId}`);
        if (deleteUser.status !== 200) {
            throw new Error("DELETE TEST USER failed.");
        }
        if (deleteUser.data?.id !== testUserId) {
            throw new Error("DELETE TEST USER returned incorrect user.");
        }
        console.log("DELETE TEST USER verified.\n");
        // =========================================================
        // VERIFY USER DELETE
        // =========================================================
        const verifyUserDelete = await request("GET", `/users/${testUserId}`);
        // Accept either 200 + null (some endpoints) or 404 Not Found.
        if (!((verifyUserDelete.status === 200 && verifyUserDelete.data === null) ||
            verifyUserDelete.status === 404)) {
            throw new Error("VERIFY USER DELETE GET failed.");
        }
        console.log("VERIFY TEST USER DELETE verified.\n");
        // =========================================================
        // SUCCESS
        // =========================================================
        console.log("==============================");
